...
public class ExpressionCalculator2 {
	...
	private void operator(ExpressionToken token) {
	...
	private void operand(ExpressionToken token) {
		switch(token.getType()) {
			case LEFT_BRACE:
				leftBrace(token);
			case RIGHT_BRACE:
	...
	private void leftBrace(ExpressionToken token) {
		//Note: Don't pop the operator just add left brace to the stack.
		stack.push(token);
	}
	
	private void rightBrace(ExpressionToken token) {
		/*
